  ...
  const services = [
    {
      ...
      details: [
        ...
        "Acceptance & Commitment Approach (ACT)",
            ...
            {services.map((service, index) => (
              <section key={index} className={`grid grid-cols-1 lg:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                <div className={`relative aspect-[3/2] rounded-2xl overflow-hidden shadow-xl ${index % 2 === 1 ? 'lg:order-last' : ''}`}>
                  <Image
                    src={service.img}
                    alt={`${service.title} session at Setu`}
                    fill
        ...
        <div className="mx-auto max-w-5xl grid grid-cols-1 md:grid-cols-3 gap-12 text-sm">
          ...
          <div>
            ...
            <div className="space-y-2 text-brand-sand/70">
              <Link href="mailto:spu.psych@gmail.com" className="block hover:text-brand-blush transition-colors" onClick={() => trackEmailClick("Services Footer")}>
                spu.psych@gmail.com
              </Link>
            </div>
          </div>
